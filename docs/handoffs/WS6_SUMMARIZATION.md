...
- [ ] **`services/summarizer.py`** — new module that becomes the single entrypoint for all summarization. Replace ad-hoc paths in `synthesis.py` and `ai_service.py`.
  ```python
  class Summarizer:
      def summarize(
          self,
  ...
  class SummaryResult:
      ...
      latency_ms: int
  ```
- [ ] **For v0.2.0, implement `mode='short'` and `mode='long'` only.** Themes and structured modes are v0.3.0.
...
- [ ] **`services/source_span_verifier.py`** — given a generated summary and the chunks it claims to summarize:
...
- [ ] **Alembic migration** adding `summary_short`, `summary_long`, `summary_themes`, `summary_computed_at`, `summary_verified` columns to `documents`:
  ```sql
  ...
  ```
- [ ] **`scripts/nightly/06b_compute_summaries.py`** — run as part of WS5a nightly, after step 06 (KG enrich), before step 07 (promote)
...
- [ ] **Backfill script** for existing documents (one-time, runs under advisory lock from WS5a)
...
- [ ] `services/summarizer.py` exists with `Summarizer.summarize` and `SummaryResult` dataclass
- [ ] `services/source_span_verifier.py` exists and is wired into `Summarizer`
- [ ] `mode='short'` and `mode='long'` working
- [ ] `documents` table has cached summary columns (Alembic migration applied)
- [ ] `scripts/nightly/06b_compute_summaries.py` runs as part of WS5a nightly
- [ ] Backfill complete for all existing Rotterdam documents
